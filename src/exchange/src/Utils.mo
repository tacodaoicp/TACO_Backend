...
module {
  ...
  public func principalToAccount(p : Principal) : AccountIdentifier {
    ...
    digest.writeArray([10, 97, 99, 99, 111, 117, 110, 116, 45, 105, 100] : [Nat8]); // b"\x0Aaccount-id"
    let blob = Principal.toBlob(p);
    digest.writeBlob(blob);
    digest.writeArray(Array.freeze<Nat8>(Array.init<Nat8>(32, 0 : Nat8))); // sub account
    let hash_bytes = digest.sum();

    return { hash = Blob.toArray(hash_bytes) } : AccountIdentifier;
  };
