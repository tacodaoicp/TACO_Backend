...
shared (init_msg) persistent actor class Example(_args : ICRC3.InitArgs) = this {
  ...
  transient let #v0_1_0(#data(icrc3_state_current)) = icrc3_migration_state;
  ...
  transient var _icrc3 : ?ICRC3.ICRC3 = null;
  ...
  public shared (msg) func addUser(user : (Principal, Text)) : async Nat {
    ...
    return icrc3().add_record<system>(#Map([("principal", #Blob(Principal.toBlob(user.0))), ("username", #Text(user.1)), ("timestamp", #Int(get_time())), ("caller", #Blob(Principal.toBlob(msg.caller)))]), ? #Map([("btype", #Text("uupdate_user"))]));
  };

  public shared (msg) func addRole(role : Text) : async Nat {
    return icrc3().add_record<system>(#Map([("role", #Text(role)), ("timestamp", #Int(get_time())), ("caller", #Blob(Principal.toBlob(msg.caller)))]), ? #Map([("btype", #Text("uupdate_role"))]));
  };
  ...
  public shared (msg) func addUserToRole(x : { role : Text; user : Principal; flag : Bool }) : async Nat {
    return icrc3().add_record<system>(#Map([("principal", #Blob(Principal.toBlob(x.user))), ("role", #Text(x.role)), ("flag", #Blob(if (x.flag) { Blob.fromArray([1]) } else { Blob.fromArray([0]) })), ("timestamp", #Int(get_time())), ("caller", #Blob(Principal.toBlob(msg.caller)))]), ? #Map([("btype", #Text("uupdate_use_role"))]));
  };
