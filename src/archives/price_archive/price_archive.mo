...
shared (deployer) persistent actor class PriceArchiveV2() = this {
  ...
  public query ({ caller }) func getLatestPrices(tokens : [Principal]) : async Result.Result<[(Principal, ?{icpPrice: Nat; usdPrice: Float; timestamp: Int})], ArchiveError> {
    if (tokens.size() > 500) return #err(#InvalidData);
    if (not base.isQueryAuthorized(caller)) {
      return #err(#NotAuthorized);
    };

    #ok(Array.map<Principal, (Principal, ?{icpPrice: Nat; usdPrice: Float; timestamp: Int})>(tokens, func(token) {
      (token, Map.get(lastKnownPrices, Map.phash, token))
    }));
  };
  ...
  // Batch query for multiple (token, timestamp) pairs — groups by timestamp to share binary searches
  public query func getPricesAtTimes(queries : [{token: Principal; timestamp: Int}]) : async Result.Result<[?{icpPrice: Nat; usdPrice: Float; timestamp: Int}], ArchiveError> {
    ...
    // Group queries by timestamp: Map<Int, Vector<(queryIndex, token)>>
    ...
    // Mutable results array, same index order as input
    let results = Array.init<?{icpPrice: Nat; usdPrice: Float; timestamp: Int}>(queries.size(), null);

    // For each unique timestamp, do one binary search + window scan
    for ((timestamp, group) in Map.entries(timestampGroups)) {
      ...
      let groupTokens = Map.new<Principal, Vector.Vector<Nat>>(); // token -> [queryIndices]
                    ...
                    switch (txKey, txValue) {
                      ...
                      case ("data", #Map(dataEntries)) {
                        if (isPrice) {
                          ...
                          switch (blockToken, blockPriceICP, blockPriceUSD) {
                            case (?bToken, ?icp, ?usdText) {
                              // Only process if this token is in our group
      ...
      // Write results back at the correct query indices
  ...
  // These exist alongside the plain query versions for callers that need
  // the composite query calling convention. The plain query versions remain
  // for rewards.mo compatibility (update functions cannot call composite queries).
  // Note: With maxActiveRecords=4M, all blocks stay local. These use the same
  // synchronous helpers as the plain query versions.
  //=========================================================================

  public composite query func getPriceAtTimeComposite(token : Principal, timestamp : Int) : async Result.Result<?{icpPrice: Nat; usdPrice: Float; timestamp: Int}, ArchiveError> {
    let sampleResult = base.icrc3_get_blocks([{ start = 0; length = 1 }]);
    let totalBlocks = sampleResult.log_length;
    if (totalBlocks == 0) return #ok(null);
                  ...
                  switch (txKey, txValue) {
                    ...
                    case ("data", #Map(dataEntries)) {
                      if (isPrice) {
                        ...
                        switch (blockToken, blockPriceICP, blockPriceUSD) {
                          case (?bToken, ?icp, ?usdText) {
                            if (Principal.equal(bToken, token) and blockTimestamp <= timestamp and blockTimestamp > mostRecentTimestamp) {
                              switch (textToFloat(usdText)) {
                                case (?usd) {
                                  mostRecentTimestamp := blockTimestamp;
                                  mostRecentPrice := ?{ icpPrice = icp; usdPrice = usd; timestamp = blockTimestamp };
                                };
                                case null {};
                              };
                            };
  ...
  public composite query func getPriceAtOrAfterTimeComposite(token : Principal, timestamp : Int) : async Result.Result<?{icpPrice: Nat; usdPrice: Float; timestamp: Int}, ArchiveError> {
    let sampleResult = base.icrc3_get_blocks([{ start = 0; length = 1 }]);
    let totalBlocks = sampleResult.log_length;
    if (totalBlocks == 0) return #ok(null);
    ...
    var bestFuturePrice : ?{icpPrice: Nat; usdPrice: Float; timestamp: Int} = null;
    var bestFutureTimestamp : Int = 9_223_372_036_854_775_807;
  ...
  public composite query func getPricesAtTimesComposite(queries : [{token: Principal; timestamp: Int}]) : async Result.Result<[?{icpPrice: Nat; usdPrice: Float; timestamp: Int}], ArchiveError> {
    if (queries.size() > 500) return #err(#InvalidData);
    if (queries.size() == 0) return #ok([]);
  ...
  public composite query func getPricesAtOrAfterTimesComposite(queries : [{token: Principal; timestamp: Int}]) : async Result.Result<[?{icpPrice: Nat; usdPrice: Float; timestamp: Int}], ArchiveError> {
    if (queries.size() > 500) return #err(#InvalidData);
    if (queries.size() == 0) return #ok([]);
